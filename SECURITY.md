...
| `CLOUDFLARE_API_TOKEN` env var | `stacks/caddy/docker-compose.yml` | Visible via `docker inspect` and Portainer UI to any authenticated Portainer user. Blast radius: stolen token allows DNS record manipulation for `longie.net` (token is scoped to DNS Edit on one zone only — no other Cloudflare permissions). Mitigated by token scope minimisation and MFA on Cloudflare account. If compromised: rotate in Cloudflare dashboard immediately. |
| Cloudflare WAF rate limiting — 1 rule only | Cloudflare dashboard (not in repo) | Free tier allows 1 rate limiting rule with 10-second minimum window. All three login endpoints (Portainer, n8n, qBittorrent) are consolidated into one rule via expression editor (5 req/10s → block). Webhook flood protection and global catch-all are not implemented. Acceptable because Cloudflare Access OAuth blocks unauthenticated users before reaching any login endpoint. Upgrade to Pro for per-service rules and 1-minute windows. |
| `configs: content:` inline block | `stacks/caddy/docker-compose.yml` | Portainer CE's manual "Pull and redeploy" button fails with `Failure [object Object]` for this stack. The 24h GitOps polling path works correctly. Workaround: force redeploy by cloning the repo on the host and running `docker compose up -d --pull always`. See `stacks/caddy/README.md` for full details. |
...
| `containrrr/watchtower:1.7.1` pinned version | `stacks/watchtower/docker-compose.yml` | Pinned to prevent supply chain auto-update risk; upgrade deliberately — check release notes before bumping |
